...
<head>
  ...
  <style>
    ...
    :root{
      --bg:#F3E3C2;      /* sand */
      --coffee:#55351D;  /* deep brown */
      --ink:#2E373D;     /* dark slate */
      --rust:#AF461F;    /* rusty red (CTA) */
      --teal:#325B56;    /* deep teal (accent) */
    ...
    *{ scroll-behavior:smooth; }
    ...
    .hero video{
      ...
      object-fit:cover; filter:contrast(1.06) saturate(1.05) brightness(.95);
    }
    .hero::after{
    ...
    .hero-content{
      ...
      padding:7rem 1rem 9rem; text-align:center; color:#fff;
      text-shadow:0 2px 20px rgba(0,0,0,.5);
    }
    .kicker{ text-transform:uppercase; letter-spacing:.28em; font-weight:600; opacity:.9; }
    .btn-pill{
      border-radius:999px; padding:.9rem 1.6rem; font-weight:600; letter-spacing:.02em;
    ...
    .scroll-cue{
      ...
      color:#fff; opacity:.85; font-size:.95rem;
    }
    ...
    /* ===== PARALLAX STRIP ===== */
    .parallax{
      position:relative; height:64vh; display:grid; place-items:center; overflow:hidden;
    ...
    .strip{
      --gap:26px; --h:190px; --w:260px; --speed:28s;
    ...
    .track{
      display:flex; gap:var(--gap); width:max-content; will-change:transform;
      animation:marquee var(--speed) linear infinite;
    }
    @keyframes marquee{ to{ transform:translateX(calc(-50% - var(--gap))); } }
    .chip{
      width:var(--w); height:var(--h); flex:0 0 auto; border-radius:18px; overflow:hidden;
    ...
    .card-elev{
      background:var(--surface); border:none; border-radius:var(--radius-xl); box-shadow:var(--shadow);
      transform:translateY(38px); opacity:0; transition:transform .45s ease, box-shadow .28s ease, filter .28s ease;
      border:1px solid rgba(46,55,61,.06);
    }
    ...
    /* ===== Maps/Lokasi preview ===== */
    .map-preview{
      position:relative; height:380px; border-radius:var(--radius-2xl);
      background:
      ...
      box-shadow:var(--shadow); border:1px solid rgba(46,55,61,.08); overflow:hidden;
    }
    ...
    .map-hint{
      position:absolute; bottom:14px; right:16px; background:#fff; color:var(--ink);
      padding:.45rem .75rem; border-radius:999px; font-size:.9rem; box-shadow:var(--shadow);
    }
    ...
    .title-underline::after{
      content:""; position:absolute; left:6px; right:-6px; bottom:-8px; height:8px; border-radius:999px;
      background:linear-gradient(90deg, var(--rust), var(--teal)); opacity:.25;
    }
      ...
      <div class="row g-5 align-items-center">
        <div class="col-lg-6 reveal">
          ...
          <p class="lead lead-muted">
            Kami memilih biji terbaik, memanggangnya dengan presisi, lalu menyeduhnya dengan penuh perhatian.
            Semua untuk menghadirkan rasa yang konsisten dari <span class="text-teal">single origin</span> hingga
            <span class="text-rust">signature blend</span>.
          </p>
          <p class="text-secondary mb-0">Terinspirasi komunitas, kami percaya setiap cangkir mampu menyambungkan
          ...
          <div class="col-12 col-md-6 col-lg-4">
            <div class="card-elev product-card position-relative">
              ...
              <div class="p-3 p-md-4">
                <h5 class="mb-1">Signature Beans #{{ $i }}</h5>
                <p class="text-secondary">
                  Origin single, roast medium, tasting notes: caramel, cocoa, hint of citrus.
                  Cocok untuk espresso & manual brew.
                </p>
                <div class="d-flex justify-content-between align-items-center mt-2">
                  <span class="fw-semibold">IDR {{ 80 + $i * 3 }}k</span>
                  <button class="btn btn-sm btn-outline-dark rounded-pill">Detail</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="map-preview position-relative">
            <div class="map-pin">
              <svg viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path fill="#AF461F" d="M172.3 501.7C26.6 291.3 0 269.5 0 192 0 86 86 0 192 0s192 86 192 192c0 77.5-26.6 99.3-172.3 309.7a24 24 0 0 1-39.4 0zM192 272a80 80 0 1 0 0-160 80 80 0 1 0 0 160z"/>
              </svg>
            </div>
            <a href="https://maps.app.goo.gl/6JfGLKPEv9M2azMA8?g_st=aw" target="_blank" rel="noopener"
